...
def makeDNAbits(instring):
    lookupdict = {'A': '0001', 'C': '0010', 'G': '0100', 'T': '1000'}
    outbits = '1'
    for c in instring:
...
def makeDNAstring(inbits):
    lookupdict = {'0001': 'A', '0010': 'C', '0100': 'G', '1000': 'T'}
    outstring = ''
    inbitsbin = '{0:08b}'.format(inbits)
    inbitsbinlist = [inbitsbin[i:i+4] for i in range(1,len(inbitsbin),4)]
    for c in inbitsbinlist:
        outstring += lookupdict[c]
    return(outstring)

def return_dist_frame(uniqumis, hamdist):
    umibits = [makeDNAbits(u) for u in uniqumis]
    tree = BKTree(hamming_distance, umibits)
    close_hits = [tree.find(umi, hamdist*2) for umi in umibits]
    outlist = []
    for b in close_hits:
        b_0 = b[0][1]
        b_rest = b[1:]
        outlist.extend([(makeDNAstring(b_0), int(b_1[0]/2), makeDNAstring(b_1[1])) for b_1 in b_rest])
    return(outlist)
